...
### 19.10 双周迭代排期（开发视角｜2 周为 1 个周期）

> 说明：从“10 万级扩展路线图”的研发实现视角拆解，优先把“多集群分区 + 分层调度 + 流式计算 + 边缘网关 + 数字孪生”跑通最小闭环，再补齐可观测、压测与演练。每个 Sprint 以“可演示闭环 + 可回滚”为标准。

#### Sprint A（第 1–2 周）：多集群分区骨架 + 统一路由网关
- [x] **目标**：形成“多集群分区”的最小可用链路（按 `warehouse_id` 路由）。
- [x] 统一路由网关雏形：`tenant_id + warehouse_id` → Cluster 路由（静态配置先行）
- [x] 控制面/数据面拆分草案：全局控制面只存索引与元数据
- [x] Topic key 分区策略定版：`robot_id/zone_id/warehouse_id`（与现有 topic 对齐）
- [x] **验收**：同一租户不同仓的请求能路由到不同集群；路由变更可回滚

#### Sprint B（第 3–4 周）：跨集群事件桥接 + 最终一致性策略
- **目标**：跨集群关键事件可同步（双写/桥接）。
- MirrorMaker2/自研 bridge POC（二选一）+ 事件双写策略
- 跨集群一致性策略定版：仅最终一致；强一致限定在单集群
- **验收**：关键事件在双集群可见且无重复副作用（幂等验证）

#### Sprint C（第 5–6 周）：分层调度 MVP（全局粗分配 + 单仓细排程）
- **目标**：调度分层跑通最小闭环。
- 全局粗分配：跨仓任务粗分配（只分目标仓/区域）
- 单仓细排程：基于本仓状态做细粒度排程
- **验收**：跨仓任务可被正确分配并在单仓内落地排程

#### Sprint D（第 7–8 周）：增量重规划 + 冲突检测分区化
- **目标**：减少全图重算、降低锁冲突。
- 增量重规划触发条件：拥堵/异常/局部变化
- 冲突检测分区化：按区域/路段执行
- **验收**：局部拥堵时只触发局部重算；全域锁冲突显著下降

#### Sprint E（第 9–10 周）：流式计算链路（窗口聚合 + 拥堵特征）
- **目标**：稳定产出 `congestion_index` 并可被调度消费。
- Flink/Kafka Streams 二选一最小实现
- 窗口聚合与特征生成（1min/5min）
- **验收**：`congestion_index` 能被实时查询与推送

#### Sprint F（第 11–12 周）：状态分层存储 + 动态降频
- **目标**：热/温/冷分层跑通，支持降频策略。
- Redis 热缓存 + Postgres 最新快照 + Influx 历史
- 动态降频策略：按 `robot_id/priority`
- **验收**：热/温/冷查询链路稳定，降频策略生效且可回滚

#### Sprint G（第 13–14 周）：边缘网关 + QoS + 断连自治
- **目标**：设备侧“边缘代理”可承压 1 万台并具备基本自治。
- 网关聚合上报/指令下发；认证与限流下沉
- QoS 规则：控制 > 任务 > 监控
- 断连自治：离线缓冲 + 短时自策略
- **验收**：1 万台设备压测通过；断连恢复不丢关键指令

#### Sprint H（第 15–16 周）：可观测与容量压测模型
- **目标**：SLO + 压测模型固化，可扩缩容。
- E2E tracing：API→调度→事件流→设备→回执
- 压测模型：Kafka/Redis/Postgres/Influx 联合压测
- HPA + Lag 联动扩缩容策略
- **验收**：关键链路 SLO 达标；扩缩容可验证

#### Sprint I（第 17–18 周）：数字孪生仿真闭环（回放 + 对齐 + 校准）
- **目标**：仿真与真实状态对齐，支持回放调参。
- 引擎选型落地（RMF sim/Gazebo/Unity/Isaac Sim）
- `robot_id` 对齐与漂移检测；回放与参数 A/B
- **验收**：可回放指定时间窗并量化误差；参数可回写

#### Sprint J（第 19–20 周）：端到端验收与分级故障演练
- **目标**：达成 19.9 验收与演练闭环。
- 10 万级状态流压测 + P99 达标验证
- 单仓/单集群/跨集群分级演练与预案
- **验收**：全部指标达到 19.9，输出演练报告
